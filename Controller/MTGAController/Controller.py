...
class Controller(ControllerSecondary):
    ...
    def _click_logout_target(self, raw_point: tuple[int, int], config_key: str, click_label: str) -> None:
        ...
        bot_logger.log_info(
            "{click_label} target: source={source} arena={self._arena_region} raw={raw_point} mapped={mapped}"
        )
    ...
    def __parse_hover_id_line(self, line):
        ...
        MTGA log formats sometimes emit full JSON lines, sometimes fragments like `"objectId": 123`.
        """
        if not line:
            return None
        try:
