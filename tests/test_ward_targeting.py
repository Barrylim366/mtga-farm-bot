...
class WardCostTest(unittest.TestCase):
    ...
    def test_coloured_ward_costs_its_pip_count(self):
        cost = RemovalLogic.ward_cost(900001)
        self.assertEqual(cost["mana"], 2)
        self.assertEqual(cost["symbols"], ["1", "U"])

    def test_coloured_ward_requires_a_matching_source(self):
        target = creature(1, 900001)
        self.assertFalse(
            RemovalLogic.ward_is_affordable(
                target, 2, [{"white"}, {"red"}]
            )
        )
        self.assertTrue(
