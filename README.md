...
- **Known gap (open):** blind hand sweeps with Arena's window *active* keep recurring across sessions and accounts, and they regularly cost the rope. Measured from the debug bundles, three of the obvious explanations are ruled out: nothing covers the hand (checked twice by screenshot); the hand band does not cut the cards off (on a seven-card hand, x=1700 lands in the middle of the rightmost card, which spans roughly 1572–1830); and the window is genuinely in the foreground. What the screenshots do show is Arena sitting in a special input mode — in one case a target selection still pending, in another a card visibly mid-cast, raised and glowing — while the controller reported `pending_card_prompt=None`, `casting_time_options_open=False` and `pending_select_n=None` at that very moment. Working hypothesis: Arena emits no hand hovers while the client is in such a mode, and the bot's state model does not know about them, so it believes a sweep is allowed. If that is right, the fix belongs in *when* a sweep is permitted, not in the recovery — the recovery is downstream of a wrong premise and can only pick the wrong remedy (reactivate a window that was never inactive, hunt for a Done button that does not exist). **And the mode is entered by the bot's own misclick**, which the first `TARGET_CLICK_INEFFECTIVE` bundle showed directly: the click for creature 299 landed at arena-relative (247, 259) — bare board, roughly the *first sample* of a scan band starting at (192, 259) — while the creature stood highlighted at about (960, 355). A hover cannot have been caused by a cursor that had not moved yet, so the sweep accepted a late event emitted for an earlier position and credited it to where the cursor happened to be. That is the same client → server → log latency the hand sweep already documents; here it leaves a target prompt open, and every hand sweep after it is blind. Selection clicks now record which sample of the sweep matched and how far in, and a match on the first two samples is logged as `SELECT_<label>_EARLY_MATCH` — the confirmation step that would actually fix it (re-hover and require a second, position-consistent report before clicking) touches the loop every hand cast depends on and is deliberately left for a supervised change
- Arena's "Report a Player" dialog is detected and **cancelled**. It is not a game prompt and no game message announces it, so the bot is blind to it: while it is up every screen probe fails and the bot burns its rope. It happened twice on 2026-08-21. The first time the trigger was found and blocked (the blind Home-tab click above); the second time there was no logged click at all between the last end-screen dismissal and the open dialog, so the trigger is still unknown — which is why this net does not depend on knowing it. The stall alone would be survivable; what makes it worth a dedicated check is the **Submit Report** button next to Cancel, pointed at a real player. Only Cancel is ever clicked: the dialog is confirmed by its title template, the button is searched for in a band that ends at x=1000 while Submit starts near x=988 and is 285px wide (so no match for it can fit), and the fallback is the measured Cancel position (797, 875), never an absolute desktop click. Both templates cross-validate against the *other* incident's capture at 0.99 (title) and 0.96 (button), well above the 0.80 threshold

  Since 2026-08-22 the probe runs from **two** places. The original one is the blind-sweep recovery path — after a full hand sweep came back empty with Arena's window active — which only exists *during* a match. That day the dialog opened on a match-end screen instead: no hand sweep ever followed, so the net never ran, and the queue loop clicked Play into the dialog for 28 minutes. There was no logged click for six minutes beforehand, so guarding another click path would again not have helped. The second hook is therefore keyed on the symptom rather than the cause: when the queue loop has ticked for 90 seconds without a match starting, it probes once per interval. That path checks the report dialog **only** — not the Done-button templates the blind-sweep recovery also tries — because it runs on Home, where a false-positive Done match would click into the live UI
