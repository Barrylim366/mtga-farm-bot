...
- A sweep that hovers *nothing at all* is treated as its own failure, not as evidence about the hand. Magic Arena runs on Unity, and Unity only reports what the mouse is over while its window is the active one — so a sweep over an inactive window crosses the whole hand row, hovers nothing, and looks exactly like a card that is not there. Observed live on 2026-08-21: from 15:56:58 on, every cast sweep ended in `SCAN_STOPPED` without a single hover event, while the discard prompts running seconds later on the *identical* coordinates hovered and clicked fine — because the fallback click that passes priority had briefly reactivated the window. The bot swept blind for ten minutes and lost a match it was winning at 22 life. Three consequences: zero hovers now logs `CAST_NO_HOVER_EVENTS` and triggers a reactivation instead of two more blind sweeps; the card is no longer marked unreachable, since a blind sweep never observed the hand; and the reactivation itself first re-focuses through the window API and only then falls back to a click on the inert background strip at the left edge of the arena
...
- Every hover scan maps its region through the game window's origin before moving the mouse. The stack/prompt scan was the one that did not: with Magic Arena windowed at x=1519 it drove the cursor to (1248, 190) and (672, 136) — across whatever else was on the desktop, hundreds of pixels left of the game — so it could never hover a stack item, and dragged the mouse out of the window while failing. The scan itself now also refuses any region that falls outside the window, so a caller that forgets to map cannot sweep the desktop
