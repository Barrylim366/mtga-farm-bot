...
def choose_blocks(
    ...
    for attacker_id in sorted(attacker_ids, key=lambda a: -power(board[a])):
        ...
        if "trample" in creature_keywords(board[attacker_id]):
            leaked[attacker_id] = max(0, power(board[attacker_id]) - result["damage_prevented"])

    def _damage_through() -> int:
        return (
            sum(power(board[a]) for a in unblocked if _hits_us(board[a]))
            + sum(damage for a, damage in leaked.items() if _hits_us(board[a]))
        )
